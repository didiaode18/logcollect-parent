...
public class LoadBalancingRpcClient extends AbstractRpcClient {
  ...
  protected void configure(Properties properties) throws EventException {
    ...
    if (hosts.size() < 2) {
      throw new EventException("At least two hosts are required to use the "
    ...
    if (lbTypeName.equalsIgnoreCase(
      ...
      try {
        ...
        throw new EventException("Unable to instantiate host selector: "
            + lbTypeName, ex);
      }
    }
  ...
  private synchronized RpcClient getClient(HostInfo info)
      throws EventException, EventDeliveryException {
    throwIfClosed();
  ...
  private RpcClient createClient(String referenceName) throws EventException {
