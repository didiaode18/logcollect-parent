...
public class ThriftRpcClient extends AbstractRpcClient {
  ...
  public void close() throws EventException {
    try {
      ...
      throw new EventException("Failed to close RPC client. ", ex);
    } finally {
      stateLock.unlock();
    }
  }
  ...
  @Override
  protected void configure(Properties properties) throws EventException {
    if (isActive()) {
      throw new EventException("Attempting to re-configured an already " +
        "configured client!");
    }
    stateLock.lock();
    try {
      ...
      throw new EventException("Error while configuring RpcClient. ", ex);
    } finally {
      stateLock.unlock();
    }
  }
  ...
  private static SSLContext createSSLContext(String truststore,
    String truststorePassword, String truststoreType) throws EventException {
    SSLContext ctx;
    try {
      ...
      throw new EventException("Error creating the transport", e);
    }
    return ctx;
  }

  private static TSocket createSSLSocket(SSLSocketFactory factory, String host,
          int port, int timeout, List<String> excludeProtocols) throws EventException {
    try {
      ...
      throw new EventException("Could not connect to " + host + " on port " + port, e);
    }
  }
}
