...
import org.apache.logcollect.EventDeliveryException;
import org.apache.logcollect.EventException;
...
implements RpcClient {
   ...
   * @throws EventException
   */
  private void connect(long timeout, TimeUnit tu) throws EventException {
    ...
    try {
      ...
      if (t instanceof IOException) {
        throw new EventException(this + ": RPC connection error", t);
      } else if (t instanceof EventException) {
        throw (EventException) t;
      } else if (t instanceof Error) {
        throw (Error) t;
      } else {
        throw new EventException(this + ": Unexpected exception", t);
      }
    }
  ...
  public void close() throws EventException {
    ...
    try {
      ...
      throw new EventException(this + ": Error closing transceiver.", ex);
    } finally {
  ...
  public synchronized void configure(Properties properties)
    ...
    try{
      if(connState == ConnState.READY || connState == ConnState.DEAD){
        throw new EventException("This client was already configured, " +
            "cannot reconfigure.");
      }
    } finally {
      stateLock.unlock();
    ...
    if (hostNames != null && !hostNames.isEmpty()) {
      ...
      throw new EventException("Hosts list is invalid: " + hostNames);
    }
    ...
    if (host == null || host.isEmpty()) {
      throw new EventException("Host not found: " + hosts[0]);
    }
    String[] hostAndPort = host.split(":");
    if (hostAndPort.length != 2){
      throw new EventException("Invalid hostname: " + hosts[0]);
    }
    Integer port = null;
    try {
      port = Integer.parseInt(hostAndPort[1]);
    } catch (NumberFormatException e) {
      throw new EventException("Invalid Port: " + hostAndPort[1], e);
    }
